...
from consolidation import consolidated_process, summary_process, format_cve_file
...
def run_operation():
    ...
    if operation not in {"consolidate", "summary", "format_cve"}:
    ...
    try:
        try:
            ...
            if operation == "format_cve":
                files = request.files.getlist("files")
                valid = [f for f in files if f and f.filename]
                if len(valid) != 1:
                    flash("Please upload exactly one consolidated CSV for 'Format CVE'.")
                    return redirect(url_for("index"))
                uploaded_file = valid[0]
                if not uploaded_file.filename.lower().endswith(".csv"):
                    flash("Only .csv is allowed for 'Format CVE'.")
                    return redirect(url_for("index"))
                ...
                output_path = format_cve_file(input_csv_path)

                download_name = custom_name or "formatted_cve.csv"
                if not download_name.lower().endswith(".csv"):
                    download_name += ".csv"
                return send_file(output_path, as_attachment=True, download_name=download_name)
            ...
            flash("Error occurred. Try uploading again or check the file type.")
            return redirect(url_for("index"))
